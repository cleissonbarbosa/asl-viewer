...
describe('Theme Utils', () => {
  ...
  describe('theme consistency', () => {
    ...
    it('should have different color values between themes', () => {
      ...
      expect(lightTheme.nodeColors.choice).not.toBe(darkTheme.nodeColors.choice);
    });

    it('should have all required node types covered', () => {
      const requiredNodeTypes = ['pass', 'task', 'choice', 'wait', 'succeed', 'fail', 'parallel', 'map'];
  ...
  describe('semantic color usage', () => {
    ...
    it('should use distinct colors for different state types', () => {
      ...
      expect(uniqueLightColors.size).toBeGreaterThanOrEqual(lightColors.length - 1);
      ...
      expect(uniqueDarkColors.size).toBeGreaterThanOrEqual(darkColors.length - 1);
    });
  });
});
